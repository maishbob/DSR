...
/**
 * Imports POS transactions from PosTrans.csv (Clarion export).
 *
 * COLUMNS IMPORTED
 *   Dailyrecordid → shift_id  (looked up via shifts.dsr_number)
 *   Date          → trans_date
 *   Amount        → amount
 *   Ref           → reference (mostly blank in legacy data)
 *
 * COLUMNS NOT IMPORTED
 *   Rec No      — legacy internal PK
 *   Postransid  — legacy POS transaction ID, not stored
 */
class ImportLegacyPosTransactions extends Command
{
    protected $signature = 'import:legacy-pos-transactions
        {station_id : The station ID to import into}
        {csv_path : Path to PosTrans.csv}
        {--dry-run : Preview without writing}';

    protected $description = 'Import legacy POS transactions from the Clarion PosTrans.csv export';
    ...
    public function handle(): int
    {
        ...
        $this->table(['Metric', 'Count'], [
            ['POS transactions created', $this->created],
    ...
    private function readCsv(string $path): array
    {
        ...
        // cardrecons uses tab separator — detect and re-parse if needed
        $sep = str_contains(implode('', $header), "\t") ? "\t" : ",";
